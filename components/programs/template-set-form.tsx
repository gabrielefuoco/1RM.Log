...
import { Plus, Trash2, ArrowDownRight, AlertTriangle, GripVertical } from "lucide-react"
...
interface TemplateSetFormProps {
    ...
    reorderSets: (startIndex: number, endIndex: number) => void
...
export function TemplateSetForm({
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (active.id !== over.id) {
            const oldIndex = setsData.findIndex((set) => set._id === active.id)
            const newIndex = setsData.findIndex((set) => set._id === over.id)
                                ...
                                <div className="space-y-2.5 pr-2">
                                    {setsData.map((set, i) => (
                                        <SortableSetItem
                                            ...
                                            set={set}
                                            index={i}
                                            updateSingleSet={updateSingleSet}
                                            removeSet={removeSet}
                                        />
                                    ))}
                                </div>
...
function SortableSetItem({
    ...
    return (
        <div
            ...
            style={style}
            className={cn(
                "bg-card/40 rounded-lg p-4 border border-border space-y-4 relative group hover:bg-card/60 transition-colors",
                isDragging && "opacity-80 scale-[1.02] shadow-xl border-primary bg-card/80"
            )}
        >
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-black text-foreground uppercase italic tracking-wider">Set {index + 1}</span>
                    {isImpossible && (
                    ...
                    {index > 0 && (
                        <button
                            onClick={() => updateSingleSet(index, { is_backoff: !set.is_backoff })}
                            className={cn(
            ...
            <div className="grid grid-cols-4 gap-3">
                {isRange ? (
                    <>
                        <div className="col-span-1 space-y-1.5 relative group/reps">
                            ...
                            <Input
                                type="number"
                                value={set.reps_min}
                                onChange={(e) => updateSingleSet(index, { reps_min: Number(e.target.value) })}
                        ...
                        <div className="col-span-1 space-y-1.5 relative">
                            ...
                            <Input
                                type="number"
                                value={set.reps_max}
                                onChange={(e) => updateSingleSet(index, { reps_max: Number(e.target.value) })}
                                className="h-9 text-sm text-center font-bold px-1 bg-background/80 border-border rounded-lg focus:border-primary/50"
                            />
                            <button
                                onClick={() => updateSingleSet(index, { _ui_mode: 'fixed' })}
                    ...
                    <div className="col-span-2 space-y-1.5 relative">
                        ...
                        <div className="relative">
                            ...
                            />
                            <button
                                onClick={() => updateSingleSet(index, { _ui_mode: 'range' })}
                ...
                <div className="col-span-1 relative space-y-1.5 text-center">
                    ...
                    <Input
                        type="number"
                        value={set.rir}
                        onChange={(e) => updateSingleSet(index, { rir: Number(e.target.value) })}
                        className={cn(
                ...
                <div className="col-span-1 space-y-1.5 relative text-center">
                    {!isBackoff ? (
                        <div className="flex items-center bg-background/50 rounded-lg p-1 border border-border w-full mb-1 h-8 relative">
                            <button
                                onClick={() => updateSingleSet(index, { _ui_weight_mode: 'absolute' })}
                                className={cn(
                            ...
                            <button
                                onClick={() => updateSingleSet(index, { _ui_weight_mode: 'percent' })}
                                className={cn(
                                    "flex-1 h-full flex items-center justify-center text-[10px] font-black uppercase tracking-wider rounded transition-all duration-200",
                                    set._ui_weight_mode !== 'absolute'
                        ...
                        {set._ui_weight_mode === 'absolute' && !isBackoff ? (
                            <div className="relative">
                                <Input
                                    ...
                                    onChange={(e) => updateSingleSet(index, {
                            ...
                            <div className="relative">
                                <Input
                                    ...
                                    onChange={(e) => updateSingleSet(index, isBackoff
