...
export function DailyReadiness({ onComplete }: DailyReadinessProps) {
    ...
    const getStatusText = () => {
        if (score <= 1) return { label: "Sfinimento", color: "text-red-500", icon: AlertTriangle }
        if (score <= 2) return { label: "Stanco", color: "text-orange-500", icon: Battery }
        if (score <= 3) return { label: "Ok", color: "text-primary", icon: Zap }
        if (score <= 4) return { label: "Carico", color: "text-green-500", icon: Dna }
        return { label: "Insuperabile", color: "text-white shadow-primary", icon: Zap }
    }
    ...
    return (
        <div className="space-y-8 p-6 bg-card border border-border rounded-3xl animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="text-center space-y-2">
                <h2 className="text-2xl font-black uppercase tracking-tighter text-white italic">Ready for Work?</h2>
                <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Valuta la tua condizione fisica e mentale</p>
            </div>
            ...
            <div className="flex flex-col items-center gap-4 py-4">
                ...
                <div className="text-center">
                    ...
                    <p className="text-[10px] text-muted-foreground font-bold mt-1 uppercase tracking-widest">Punteggio: {score}/5</p>
                </div>
                <Slider
                    value={[score]}
                    onValueChange={(v) => {
            ...
            {score <= 2 && (
                <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                    <p className="text-[10px] text-orange-500 font-black uppercase tracking-widest text-center">Consigliamo un taglio al carico o al volume</p>
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        >
                            <span className="text-xs font-black uppercase mb-1">Volume Cut</span>
                            <span className="text-[8px] font-bold text-center">-1 set per esercizio</span>
                        </button>
                        <button
                        ...
                        >
                            <span className="text-xs font-black uppercase mb-1">Intensity Cut</span>
                            <span className="text-[8px] font-bold text-center">-10% carico suggerito</span>
                        </button>
                    </div>
                </div>
            )}
            ...
            >
                Inizia Allenamento <Check className="ml-2 h-5 w-5" />
            </Button>
        </div>
    )
}
